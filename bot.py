...
BOT_TOKEN = "8561589587:AAG4fJwGulD19__E0HAZH4K2jk5odT8hndg"
OWNER_ID = 1008312062
...
@dp.message(F.message_thread_id == 549, Command(commands=['stats', 'report']))
        ...
        if not user_stats:
            ...
            for i, (user_id, stats) in enumerate(sorted_users):
                if stats['name'] is None:
                    try:
                        user = await bot.get_chat(user_id)
                        stats['name'] = f"@{user.username}" if user.username else user.first_name or f"ID{user_id}"
                    except:
...
@dp.message(F.message_thread_id == 549)
async def handle_post_link(message: Message):
    ...
    bot_message = await bot.send_message(
        chat_id=message.chat.id,
        message_thread_id=549,
        text=f"🔗 Пост #{tasks[task_id]['task_num']}:\n{url}\n\n👥 0 выполнили",
...
async def confirm_reaction(callback: CallbackQuery):
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"Засчитано ({count})", callback_data=f"confirm_{task_id}")]
    ])
...
async def show_stats(callback: CallbackQuery):
    ...
    task = tasks[task_id]
    await callback.answer(f"Выполнили: {task['reaction_count']}", show_alert=True)
